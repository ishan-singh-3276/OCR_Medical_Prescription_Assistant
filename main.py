...
st.write("Get personalized medical assistance based on your prescription.")
...
if not st.session_state.patient_info_completed:
    st.markdown("---")
    st.markdown("### 📋 Patient Information")
    st.write("Please provide some information about yourself to help personalize your medical assistance.")
    ...
    st.markdown("---")
    st.info("⚠️ Please complete the patient information form above before uploading your prescription.")
else:
    # Show patient info summary and allow editing
    with st.expander("👤 View/Edit Patient Information", expanded=False):
        col1, col2 = st.columns(2)
        ...
        if st.button("Edit Patient Information"):
            st.session_state.patient_info_completed = False
            st.rerun()

# Image uploader
if st.session_state.patient_info_completed:
    upload_image = st.file_uploader("Upload a prescription image", type=["png", "jpg", "jpeg"])
else:
    upload_image = None
    st.warning("Please complete the patient information form above to proceed.")

if upload_image is not None and st.session_state.patient_info_completed:
    # Show uploaded image
